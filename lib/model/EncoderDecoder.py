#https://github.com/lanwuwei/SPM_toolkit/blob/master/ESIM/Tree_IM/model.py
import torch
...
class Encoder_W2V(nn.Module):
    ...
    def forward(self, inputs, hidden=None):
        ...
        # print "inputs[1]: "
        # print inputs[1]
        # print "self.word_lut(inputs[0]), inputs[1]: "
        # print self.word_lut(inputs[0]), inputs[1]
...
class HybridDecoder(nn.Module):
    def __init__(self, opt, dicts):
        ...
        self.rnn = StackedLSTM(opt.layers, input_size, opt.rnn_size, opt.dropout)
        if opt.has_attn:
            # self.code_attn = lib.GlobalAttention(opt.rnn_size)
    ...
    def step(self, emb, output, hidden_tree, context_tree, hidden_txt, context_txt):
        ...
        if self.opt.has_attn:
            ...
            # print output
            # print("attn_tree: ", re.sub('[\[\]]', '', np.array_str(attn_tree.data.cpu().numpy()).replace('\n','')))
            # print("attn_txt: ", re.sub('[\[\]]', '', np.array_str(attn_txt.data.cpu().numpy()).replace('\n','')))
        else:
...
class Hybrid2SeqModel(nn.Module):
    ...
    #     #  the encoder hiA tutorial on active learningdden is  (layers*directions) x batch x dim
    ...
    def initialize(self, inputs, eval):
        ...
        # init_output = self.make_init_decoder_output(enc_context_padded_tree)
        # print "enc_context_padded_tree: "
        # print enc_context_padded_tree
        # print "init_output: "
        # print init_output
        # text encoder
        ...
        # print "enc_context_txt: "
        # print enc_context_txt
        # print "-init_output: "
        # print init_output

        # enc_hidden_txt = (self._fix_enc_hidden(enc_hidden_txt[0]), self._fix_enc_hidden(enc_hidden_txt[1]))
        ...
        # return tgt, (emb, init_output, enc_hidden_txt, enc_context_txt.transpose(0, 1))

        # enc_hidden_tree = (self._fix_enc_hidden(enc_hidden_tree[0]), self._fix_enc_hidden(enc_hidden_tree[1]))
...
class Seq2SeqModel(nn.Module):
    ...
    def initialize(self, inputs, eval):
        ...
        enc_hidden = (self._fix_enc_hidden(enc_hidden[0]), self._fix_enc_hidden(enc_hidden[1]))
        # init_token = Variable(torch.LongTensor([lib.Constants.BOS] * init_output.size(0)).unsqueeze(1), volatile=eval)
        init_token = Variable(torch.LongTensor([lib.Constants.BOS] * init_output.size(0)), volatile=eval)

        if self.opt.cuda:
            init_token = init_token.cuda()
        emb = self.decoder.word_lut(init_token)
        # input = init_token.data.cpu().numpy()
        # emb = self.decoder.embedding(input).squeeze(1)
    ...
    def translate(self, inputs, max_length):
        ...
        for i in range(max_length):
            ...
            emb = self.decoder.word_lut(Variable(pred))
            # emb = self.decoder.embedding(pred.unsqueeze(1).cpu().numpy()).squeeze(1)
