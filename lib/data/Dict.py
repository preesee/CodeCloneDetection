...
class Dict(object):
    ...
    #@dispatch(self,**kwargs)#str, str=None)
    def add(self, label, idx=None):
        ...
        return idx
    #@dispatch(CommonTokenStream, str=None)
    def add_j(self, label, idx=None):
        "Add `label` in the dictionary. Use `idx` as its index if given."
        #label = label.lower if self.lower else label
        if idx is not None:
            self.idxToLabel[idx] = label.text
            self.labelToIdx[label] = idx
        else:
            if label.text in self.labelToIdx:
                idx = self.labelToIdx[label.text]
            else:
                idx = len(self.idxToLabel)
                self.idxToLabel[idx] = label.text
                self.labelToIdx[label.text] = idx
