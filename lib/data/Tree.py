...
# https://github.com/ttpro1995/TreeLSTMSentiment/blob/master/tree.py
import lib
# from lib.parser.JavaParser import JavaParser
import codecs
import lib.data.Constants as Constants
from .Dict import Dict
import ast, asttokens
from lib.parser.JavaLexer import JavaLexer
from lib.parser.JavaParser import JavaParser
...
def makeVocabulary(opt, name, filename, size):
    ...
    print("opt.lower: ", True)
    print(Constants.PAD_WORD, Constants.UNK_WORD, Constants.BOS_WORD, Constants.EOS_WORD)

    vocab = Dict([Constants.PAD_WORD, Constants.UNK_WORD, Constants.BOS_WORD, Constants.EOS_WORD], lower=True)
...
def initVocabulary(opt, name, dataFile, vocabSize):
    ...
    # vocab = genWordVocab
    return makeVocabulary(opt, name, dataFile, vocabSize)
...
def traverse_python_tree(atok, root):
    ...
    for node in asttokens.util.walk(root):
        ...
        node_json["%s%s" % (Constants.NODE_FIX, current_idx)] = {"node": type(node).__name__, "children": [],
                                                                 "parent": None}
        ...
        if not next(iter_children(node), None) is None:
            ...
            for child in iter_children(node):
                child_idx += 1
                node_json["%s%s" % (Constants.NODE_FIX, current_idx)]['children'].insert(0, "%s%s" % (
                    Constants.NODE_FIX, current_global[current_idx] - child_idx + 1))
        else:  # leaf node
...
def split_tree(tree_json, idx_upper):
    ...
    for k, node in tree_json.items():
        if len(node['children']) > 2:
            tree_json_splited['%s%s' % (Constants.NODE_FIX, idx_upper + 1)] = {'node': 'Tmp',
                                                                               'children': node['children'][1:],
                                                                               'parent': k}  # idx_upper + 2
            for ch in node['children'][1:]:
                tree_json_splited[ch]['parent'] = '%s%s' % (Constants.NODE_FIX, idx_upper + 1)
            tree_json_splited[k]['children'] = [tree_json_splited[k]['children'][0],
                                                '%s%s' % (Constants.NODE_FIX, idx_upper + 1)]  # idx_upper + 2
